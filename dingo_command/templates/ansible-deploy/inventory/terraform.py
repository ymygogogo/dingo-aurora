...
def get_network_info(filenames):
    for filename in filenames:
        with open(filename, 'r') as json_file:
            ...
            for resource in tfstate['resources']:
                if resource['type'] == 'openstack_networking_subnet_v2' and resource['name'] == 'bus_k8s_output':
                    for instance in resource['instances']:
...
def main():
    ...
    # 获取网络信息
    network_info = get_network_info(tfstates(args.root))
    
    # 将网络信息添加到每个主机的 hostvars
    ...
    if args.list:
        ...
        if args.nometa:
            del output['_meta']
        for host in output['_meta']['hostvars']:
            output['_meta']['hostvars'][host].update({
            'network_cidr': network_info['cidr'],
            'network_gateway': network_info['gateway_ip']
        })
